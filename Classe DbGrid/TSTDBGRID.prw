...
User Function TSTDBGRID()
	...
	*/
	...
	//----------------------------------------------------------------------------------------------------------------//
	oDbGrid := DbGrid():Create(nTop, nLeft, nBottom, nRight, GD_INSERT + GD_UPDATE + GD_DELETE , cLinhaOK, cTudoOK, cIniCpos, aAlterGDa, , nMax, cFieldOK, cSuperDel, cDelOK, oPanel1, aHeader, aCols, 1, 0, {|| MsgAlert('NA CRIACAO')}/*, 'B1_COD', 5, '0000'*/)

	//oDbGrid:Refresh()
	oDbGrid:hDuplClk := {|| MsgAlert('TESTE')}
...
User Function TSTTudOK()
	...
	If nDel == Len(oDbGrid:aCols)
	   MsgAlert("Para excluir todos os itens, utilize a op็ใo EXCLUIR", 'Valida็ใo de Grava็ใo')
	   lRet := .F.
	EndIf
