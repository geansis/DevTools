...
/*/{Protheus.doc} DEVHEL01
@description Gerador de script em txt para auxiliar no desenvolvimento.
...
User Function DEVHEL01()
	...
	oPanelh1   := TPanel():New( 000,000,"",oDlgh1,,.F.,.F.,,,223,035,.F.,.F. )
...
User Function HGerCodigo(pArquivo, pCaminho, pTipo)
	...
	If Empty(pArquivo)
		MsgAlert('Por favor, informe o Arquivo - Existente SX3!')
		Return
	EndIf

	If Empty(pCaminho)
		MsgAlert('Por favor, informe o Destino do Script!')
		Return
	EndIf

	If Empty(pTipo)
		MsgAlert('Por favor, informe o Tipo do arquivo de Script!')
		Return
	EndIf
	...
	If pTipo = 1   // Geração da Struct de tabela para criacao de tabela temporaria
		...
		If Found()
			...
			lGer := .t.
		Else
			MsgAlert('Arquivo ' + pArquivo + ' não encontrado! ')
			Return
		EndIf
	...
	If lGer
		MsgAlert('Arquivo de Script gerado com sucesso!')
	...
	Else
		MsgAlert('Arquivo de Script não foi gerado!')
	EndIf
...
Static Function MsgSelEmp()
	...
	bOk2 := {|| IIF(cCodEmp <> Space(2) .or. cCodFil <> Space(8), SetaEmp(), MsgAlert('Por favor, informe a Empresa e a Filial!'))  }
